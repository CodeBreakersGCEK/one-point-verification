      ...
      <div className="flex items-center  justify-start flex-col" >
        ...
        <div className='h-1 w-24 mt-2 bg-sky-700'  ></div>
         ...
         <div className='mt-16 flex gap-1 items-center justify-around'  >
            {
                actions.map((action)=>(
                    <div className='w-80 h-full p-6 px-4 bg-[#C5EAFF] flex flex-col items-center scale-100 hover:scale-110 transition-transform ease-out delay-400 cursor-pointer' key={action.id} >
                                <img src={action.src} alt={action.title} className='my-9' />
                               <h3 className='text-2xl font-popins font-medium my-4 ' >{action.title}</h3>
